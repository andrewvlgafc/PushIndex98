...
The DDK approach was different: a kernel-mode filter driver that sat below the I/O manager and intercepted filesystem operations at the IRP level. When a file was written, the driver intercepted the IRP_MJ_WRITE request at the IRP level, mapped the physical file path to a URL via a configurable routing table, and triggered a user-mode service.


That service did three things in sequence: (1) extracted visible text and anchor links from the HTML file using a lightweight parser I wrote because I couldn't fit a full browser engine on a Pentium 60 with 8MB RAM; (2) compared the extracted content with the last known state stored in a local file index in its own binary format; (3) compressed only the delta using zlib and packaged it into a small binary payload with a custom header containing the URL, last-modified timestamp, and content-type.
